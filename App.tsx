...
import { LayoutDashboard, FileText, Calculator, Receipt, Banknote, ShieldCheck, ChevronRight, Zap, RefreshCw, Send, ListChecks } from 'lucide-react';
import { WorkflowStatus, RevenueRecord } from './types';
import { MOCK_PROJECTS, INITIAL_RECORDS, MOCK_SUBJECT_MAPPINGS } from './constants';
...
const App: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'dashboard' | 'contract' | 'mapping' | 'accrual' | 'settlement' | 'invoice' | 'clearing'>('dashboard');
  ...
  const navItems = [
    ...
    { id: 'mapping', label: '科目映射', icon: ListChecks },
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'mapping': return <SubjectMappingSection mappings={MOCK_SUBJECT_MAPPINGS} />;
      case 'accrual': return <AccrualProcess records={records} onAccrue={(id, amt) => simulateProcess('暂估记账', () => updateRecordStatus(id, { status: WorkflowStatus.ACCRUED, accrualAmount: amt, accrualDate: new Date().toISOString() }))} />;
      case 'settlement': return <SettlementProcess records={records} onSettle={(id, amt) => simulateProcess('生成结算单', () => updateRecordStatus(id, { status: WorkflowStatus.SETTLED, settlementAmount: amt, settlementDate: new Date().toISOString() }))} />;
      ...
      <aside className="w-64 bg-slate-900 text-white flex flex-col shadow-2xl">
        <div className="p-6 flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 font-medium">业财一体化平台</p>
          </div>
        </div>
          ...
          {navItems.map((item) => (
            <button
              ...
              className={`w-full flex items-center space-x-3 px-3 py-2.5 rounded-lg transition-all duration-200 ${
                activeTab === item.id ? 'bg-emerald-600 text-white shadow-lg' : 'text-slate-400 hover:text-white hover:bg-slate-800'
              }`}
            >
              <item.icon size={20} />
        ...
        <div className="p-4 border-t border-slate-800">
          <div className="bg-slate-800 rounded-xl p-3 flex items-center space-x-3">
            ...
            <div className="flex-1 overflow-hidden">
              <p className="text-sm font-semibold truncate">财务主管</p>
              <p className="text-[10px] text-slate-500 truncate uppercase tracking-wider">GLP Finance Admin</p>
            </div>
          </div>
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        <header className="h-16 bg-white border-b border-slate-200 px-8 flex items-center justify-between shadow-sm z-10">
          ...
          <div className="flex items-center space-x-6">
             {loading && (
               <div className="flex items-center space-x-2 text-emerald-600 font-bold text-xs animate-pulse">
                 <RefreshCw size={14} className="animate-spin" />
                 <span>数据同步中 (SAP API)...</span>
               </div>
             )}
             <div className="flex items-center space-x-2 bg-slate-50 px-3 py-1.5 rounded-full border border-slate-100">
               <ShieldCheck size={16} className="text-emerald-500" />
               <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">TLS 1.3 数据加密</span>
             </div>
          </div>
        </header>
